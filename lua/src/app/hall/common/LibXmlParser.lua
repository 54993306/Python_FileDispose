-----------------------------------------------------------------------------------------
-- LUA only LibXmlParser from Alexander Makeev
-----------------------------------------------------------------------------------------
LibXmlParser = {};

function LibXmlParser:ToXmlString(value)
...
function LibXmlParser:FromXmlString(value)
...
function LibXmlParser:ParseArgs(s)
	local arg = {}
...
function LibXmlParser:ParseXmlText(xmlText)
	local stack = {}
	...
	while true do
		...
		elseif c == "" then   -- start tag
			...
			top = stack[#stack]
			if #stack < 1 then
				error("LibXmlParser: nothing to close with "..label)
			end
			if toclose.Name ~= label then
				error("LibXmlParser: trying to close "..toclose.Name.." with "..label)
			end
	...
	if #stack > 1 then
	--	error("LibXmlParser: unclosed "..stack[#stack].Name)
	end
