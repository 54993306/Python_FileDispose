...
function ClubRoomListWnd:initTipsArrow(data)
    Log.i("ClubRoomListWnd:initTipsArrow")
    -- if not IsPortrait then
    --     return
    -- end
...
function ClubRoomListWnd:refreshRoomList(roomsData, showType)
    ...
    for i,data in ipairs(roomsData) do
        ...
        if gameName ~= "" then
            ...
            lab_rulerInfo:setString(string.format("%d局",data.roRC))

            local lab_roomNum = ccui.Helper:seekWidgetByName(lay, "lab_roomNum")
            if (data.roI ~= nil and data.roI ~= 0) then
                if IsPortrait then
                    lab_roomInfo:setString(string.format("%s(%d人房)", gameName, data.roAUC))
                else
