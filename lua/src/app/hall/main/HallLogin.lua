...
function HallLogin:checkWhiteLists(tData, onComplete)
    ...
    Log.i("self:isMaintain", self:isMaintain(tData))
    if not self:isMaintain(tData) then
        -- onComplete()
        if self:isWhiteUpdateList(tData) then
            LoadingView.getInstance():hide()
            -- onComplete()
    ...
    Log.i("self:inWhiteLists(tData.gameWhiteList)", self:inWhiteLists(tData.gameWhiteList))
    if self:inWhiteLists(tData.gameWhiteList) then
        onComplete()
