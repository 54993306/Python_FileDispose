...
function  MjGroups:addJiaGangGroup(content)
    ...
    for i=1,#self.mjsGroupsQueue[content.operator] do
   		...
   		-- 判断之前是否有碰过这个牌, 默认取麻将最后的牌对比是否是需要加杠的牌,还有判断是否是同一个操作者
        --  当存在癞子牌时，将癞子牌排在前面
   		if objContent.actionType 	== enOperate.OPERATE_PENG 
   			and objContent.mjs[1] 	== content.mjs[4]
            ...
            elseif playerCount == 3 and operator == 3 then
   			...
   			local mjElement = Mj.new(kMjType[operator][2], content.mjs[1])  --  有癞子牌的情况下，第一位置存癞子牌，补杠癞子
			...
			elseif operator == enSiteDirection.SITE_OTHER then
   			...
   			table.insert(objContent.mjs, content.mjs[1]) 	--  有癞子牌的情况下，第一位置存癞子牌，补杠癞子
            jiaGangSuccess = true
            -- 统计打出去的牌对象
            table.insert(self.putOutObjs, mjElement)
            if not IsPortrait then -- TODO
                table.insert(self.mjsGroupsQueue[content.operator][i].putOutObjs, mjElement)
            end
