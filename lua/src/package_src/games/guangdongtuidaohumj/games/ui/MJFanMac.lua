--翻马UI
MJFanMac = class("MJFanMac", UIWndBase);

function MJFanMac:ctor(data, ...)
    self.super.ctor(self, "package_res/games/guangdongtuidaohumj/game/fangma.csb", ...);
...
function MJFanMac:onClose()

end

function MJFanMac:onInit()
...
function MJFanMac:createAnimation()
       ...
       --先执行位移动作
		...
		local callFunc = cc.CallFunc:create(function () 
			...
			local function animationFinish()
                ...
                local overdata = lGameSystem:getGameOverDatas()
                --Log.i("推倒胡结算人数datas",overdata)
                --overdata.huCount
...
function MJFanMac:keyBack()
--package.loaded["app.games.common.ui.gameover.FriendOverView"] = nil
--    package.loaded["app.games.hongzhongmj.games.ui.gameover.FriendOverView"] = nil
--    FriendOverView = require "app.games.hongzhongmj.games.ui.gameover.FriendOverView"
--UIManager:getInstance():popWnd(FriendOverView)
end
function MJFanMac:playLightAnimation(bg,mj)
    local tmpBg= bg;
	local tmpMj = mj;
	local function animationFinish()
