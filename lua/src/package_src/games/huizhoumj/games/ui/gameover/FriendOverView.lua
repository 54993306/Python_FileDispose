...
-- dump(wanfa, "sssssssssssssssssssss")
...
function FriendOverView:addPlayers()
    ...
    --------------------
    local allFanMa = self.gameSystem:getGameStartDatas().horseCards
    -----------------------
    for i = 1, #players do
        ...
        if site  and  zhuangSite ~= site and #players > 2 then
            local imgZuiDetail = display.newSprite("package_res/games/huizhoumj/games/site_detail.png")
            item:addChild(imgZuiDetail)
            imgZuiDetail:setLocalZOrder(100)
            imgZuiDetail:setPosition(cc.p(52,45))
        ...
        if #scoreitem.maCards > 0 or scoreitem.showMaCards == 1  then
            print("<jinds>： #scoreitem.maCards", #scoreitem.maCards)
            print("<jinds>： #scoreitem.scoreitem.showMaCards", scoreitem.showMaCards)
            line:setVisible(true)
            self:showZhongMa(scoreitem.maCards, hua_mj, allFanMa)
        end
...
function FriendOverView:initHuImage(item,scoreitems)
    ...
    if scoreitems.result == enResult.WIN then --胡牌玩家
        ...
        elseif self.gameOverDatas.winType == enGameOverType.QIANG_GANG_HU then
            img_hu:loadTexture("package_res/games/huizhoumj/games/qiangganghu.png", ccui.TextureResType.localType)
        else
...
function FriendOverView:showFlower(flowerCards, parent)
    ...
    if flowerCards and #flowerCards > 0 and parent then
        ...
        local maSign = display.newSprite("package_res/games/huizhoumj/games/img_hua_hor.png")
        maSign:addTo(parent)
        ...
        for i,k in pairs(flowerCards) do
            ...
            local index_x = (i - 1)%12 + 2--向后偏移两个位置
            flowSp:setPosition(cc.p((mjSize.width + 1) * index_x + mjSize.width * 0.5 + 10, 40))
            flowSp:addTo(parent)
...
-- flowerCards 花牌的牌值
-- parent 花牌的父节点
function FriendOverView:showZhongMa(maCards, parent, allFanma )
    dump(maCards, "<jinds>: maCards   ")
    if allFanma and #allFanma > 0 and parent then
        local maSp = Mj.new(enMjType.MYSELF_PENG, 11)
        local mjSize = maSp:getContentSize()
        local maSign = display.newSprite("package_res/games/huizhoumj/games/img_ma_hor.png")
        maSign:addTo(parent)
        maSign:setAnchorPoint({0,0})
        maSign:setPosition(0, 45)
        ...
        for i,k in pairs(allFanma) do
            ...
            local index_x = (i - 1)%10+2
            maSp:setPosition(cc.p((mjSize.width + 1) * index_x + mjSize.width * 0.5 + 10, 80 ))
            maSp:addTo(parent)
            
            -- for _,v in ipairs(maCards) do
            if tableFind(maCards, k) then
                -- Log.i("<jinds>: true", maSp:getAnchorPoint())
...
function FriendOverView:showAniTurnCard()
    ...
    for i, v in ipairs(startdata.horseCards) do
        local item = { CardID = v, Lottery = 0 }
    ...
    for i = 1, #players do
        ...
        if not mineZhong then
            for j = 1, #scoreitem.maCards do
                table.insert(allLightMa, scoreitem.maCards[j])
            end
        else
            if scoreitem.userid ==  self.myUserid then
                for j = 1, #scoreitem.maCards do
    ...
    --  没有翻牌数据则直接跳过翻牌动画
    if lGameOverData.winType == 3 or #lTurnCardList == 0 then --流局了 
