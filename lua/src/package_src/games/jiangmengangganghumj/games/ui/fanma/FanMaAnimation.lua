...
function FanMaAnimation:addCache()
   	cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/jiangmengangganghumj/game/fangma.plist")
   	cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/jiangmengangganghumj/game/fanmalight.plist")
end
...
function FanMaAnimation:drawFanMaText(bgLayer)
    local fmt = display.newSprite("package_res/games/jiangmengangganghumj/game/fanma_text.png")
    fmt:addTo(bgLayer)
    local bgCS = bgLayer:getContentSize()
    fmt:setPosition(cc.p(bgCS.width/2,bgCS.height+fmt:getContentSize().height))
end
--绘制头像
...
function FanMaAnimation:drawBg(index,rsNum)
    ...
    bgLayer:addTo(self)
    --local mjs = self.m_data[index].fanma or {21,22,23,24}
    local mjs = self.m_data.fanMa or {21,22,23,24}
    local zhongma = self.m_data.zhongMa or {}
    if #mjs > 0 and not self.m_quanma then
        self:drawFanMaMj(bgLayer,mjs,zhongma,rsNum)
    elseif self.m_quanma then
        local quanma = display.newSprite("package_res/games/jiangmengangganghumj/game/quanma_text.png")
...
--                      if (#mjs)%2 == 1 then
--                --posX = display.cx + (-(-1)^(i-1))*(sprite:getContentSize().width+10)*(math.modf(i/2))
--                     --else
--                --posX = display.cx + (-(-1)^(i-1))*(sprite:getContentSize().width+10)*(math.modf(i/2))-0.5*(sprite:getContentSize().width)
--                      end

--       else --#mjs>=8 and #mjs<30 then

--                   local scale = 1-((#mjs)-8)*0.045
--                  sprite:setScale(scale)
--                posX = display.cx +scale*(-(-1)^(i-1))*(sprite:getContentSize().width+10)*(math.modf(i/2))-0.5*scale*(sprite:getContentSize().width)
...
--    local df = cc.DelayTime:create(0.5+#mjs*0.32)
--    local cf = cc.CallFunc:create(function()
--        Log.i("drawEffectAnimation.......................")
--        self:removeFromParent() 
--        MjMediator.getInstance():getEventServer():dispatchCustomEvent(enMjNtfEvent.GAME_ACT_ANIMATE_FINISH_NTF,"AnimationHU")
--    end)
