...
function GameRoomInfoUI_ddztwop:initDDZTWOP()
    ...
    local group1 = SelectRadioPanel.new(kRule.personDatas, function(index)
        if index == 1 then
            package.loaded["package_src.games.paodekuai.pdktwop.GameConfig"] = nil
            require("package_src.games.paodekuai.pdk.GameConfig")
        else
            package.loaded["package_src.games.paodekuai.pdk.GameConfig"] = nil
            require("package_src.games.paodekuai.pdktwop.GameConfig")
        end
