    ...
    self.m_pWidget:performWithDelay(function()
        ...
        self.m_pWidget:performWithDelay(function()
            ...
            for i,v in pairs(PlayerModelList) do
                ...
                for j,jv in pairs(packetInfo.AGR.playFenMap) do
                    if tostring(j) == tostring(userId) and tonumber(jv) ~= 0 then
                        ...
                        self:SettlementAnimation(seat,jv - DataMgr:getInstance():GetPlayerScore(userId),2.5,0.5,addMoney)
            ...
            self.m_pWidget:performWithDelay(function()
                ...
                Log.d("--wangzhi--disMissFlag--",disMissFlag,nowJuCnt,juTotal)
                -- 判断游戏是否结束
...
function DDZRoom:DrawGameOVerTimer()
    ...
    if HallAPI.DataAPI:getJuNowCnt() > HallAPI.DataAPI:getJuTotal() then
    ...
    local function drawLabel()
        if self.label_timer and not tolua.isnull(self.label_timer) then
...
function DDZRoom:onClickButton(pWidget, EventType)
    if EventType == ccui.TouchEventType.ended then
        ...
        elseif pWidget == self.btn_menu then
            ...
            NativeCallUmengEvent(UmengClickEvent.PDKMoreButton)
        ...
        elseif pWidget == self.btn_chat then
            ...
            NativeCallUmengEvent(UmengClickEvent.PDKGameChatText)
        ...
        elseif pWidget == self.btn_jiesan then
            NativeCallUmengEvent(UmengClickEvent.PDKGameDissmissRoom)
            self:jiesanBtnEvent()
        ...
        elseif pWidget == self.btn_record then
            NativeCallUmengEvent(UmengClickEvent.PDKGameRecord)
...
function DDZRoom:onTouchSayButton(pWidget, EventType)
    ...
    if EventType == ccui.TouchEventType.began then
        NativeCallUmengEvent(UmengClickEvent.PDKGameVoiceInput)
