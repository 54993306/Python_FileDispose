...
local DDZMyHandCardView = require("package_src.games.paodekuai.pdk.mediator.widget.DDZMyHandCardView")
local DDZRightHandCardView = require("package_src.games.paodekuai.pdk.mediator.widget.DDZRightHandCardView")
local DDZLeftHandCardView = require("package_src.games.paodekuai.pdk.mediator.widget.DDZLeftHandCardView")
...
function DDZHandCardView:showOpration(info)
    ...
    if tuoguanState == DDZConst.TUOGUAN_STATE_0 then
        ...
        self.m_handCardView:zidongChuPai()
    end
end
...
function DDZHandCardView:onPlayCard(info, isReconnect)
    ...
    if not isReconnect and cards and #cards >= 0 then
        ...
        if self.card_num_img then
            ...
            if self.card_num <= CARD_REMAIN_NUM_1 and self.card_num > 0 then
                self:showJingdeng();
            else
                self:clearJingdeng()
            end
...
function DDZHandCardView:updateOpration(info)
    ...
    elseif info.action == "tishi" then
        Log.i("--wangzhi--onTiShiClick--点击了提示--")
...
function DDZHandCardView:dealCard(isReconnect)
    ...
    Log.i("--wangzhi--dealCard--wanfa--",wanfa)
    if wanfa~=nil and next(wanfa) then
        for i,v in ipairs(wanfa) do
            if v == "xianshipai" then
                self.isShowCardNum = true
            end
        end
    end
    -- Log.i("--wangzhi--self.isShowCardNum--",self.isShowCardNum)
    -- self.card_num_img:setVisible(self.isShowCardNum)
    ...
    elseif self.m_rightHandCardView then
        ...
        local isPorkerNumber = DataMgr:getInstance():isVisitPokerNumber()
        if self.card_num_img and isPorkerNumber and self.isShowCardNum then
            self.card_num_img:setVisible(true);
            self.card_num_img:setString("" .. self.card_num);
        end
    elseif self.m_leftHandCardView then
        ...
        -- local isPorkerNumber = DataMgr:getInstance():isVisitPokerNumber()
        if self.card_num_img and self.isShowCardNum then
            self.card_num_img:setVisible(true);
            self.card_num_img:setString("" .. self.card_num);
        end
    end  
