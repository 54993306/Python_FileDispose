...
function PokerToast:ctor()
    self.m_delay = 0
    self.m_pWidget = ccs.GUIReader:getInstance():widgetFromBinaryFile("package_res/games/pokercommon/null_layer.csb");
...
function PokerToast:show(text)
    ...
    local toast = ccs.GUIReader:getInstance():widgetFromBinaryFile("package_res/games/pokercommon/toast.csb");
    ...
    testHeight:setFontName("package_res/games/pokercommon/font/fangzhengcuyuan.TTF");
    local size = testHeight:getContentSize();

    label:setString(text or "提示");
    label:setFontName("package_res/games/pokercommon/font/fangzhengcuyuan.TTF");
