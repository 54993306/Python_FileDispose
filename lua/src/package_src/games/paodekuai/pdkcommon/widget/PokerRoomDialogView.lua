...
local dialogViewPath ="package_src.games.paodekuai.pdkcommon.widget.PokerRoomDialogView"
...
function PokerRoomDialogView:ctor(data, zOrder, delegate)
    ...
    self.super.ctor(self,"package_res/games/pokercommon/tip_view.csb", data, zOrder, delegate);
end
...
function PokerRoomDialogView:onInit()
    self.title = self.m_data.title and self.m_data.title or ""
    self.btn_title = ccui.Helper:seekWidgetByName(self.m_pWidget,"lbl_title")
    self.btn_title:setString(self.title)
