...
function MJFanMa:getMaxCount(data)
	...
	for k,v in pairs(data) do
		...
		local row=math.ceil(#v.faI9/self.m_num)
		totalRow=totalRow+row

		if #v.faI9>maxCount then
			maxCount=#v.faI9
		end
	end
...
function MJFanMa:onInit()
    ...
    cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/guangdongjihumj/game/fama/fangma.plist")
    cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/guangdongjihumj/game/fama/light.plist")
    cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/guangdongjihumj/game/fama/explode.plist")

    local title=display.newSprite("package_res/games/guangdongjihumj/game/fama/title.png")
    ...
    for i = 1, playerCount do
		...
		local fanmaList = self.m_data[i].faI9;
		if #fanmaList>0 then
		...
		for k,v in pairs(self.m_panels[index].famas) do
				...
				for _,fama in pairs(v) do
					...
					if famas[k] and fama:getColor().r~=120 then --根据颜色判断是否是中的马
						...
						local famaNumBg=display.newSprite("package_res/games/guangdongjihumj/game/fama/maCount1.png")
						totalFama:addChild(famaNumBg)
						famaNumBg:setPosition(103, 161)

                        local famaNum = cc.Label:createWithTTF(#v, "hall/font/fangzhengcuyuan.TTF", 24)
                        local famaNumBgSize = famaNumBg:getContentSize()
                        famaNum:addTo(famaNumBg)
                        famaNum:setPosition(famaNumBgSize.width - 14, famaNumBgSize.height - 12)
                        famaNum:setAnchorPoint(cc.p(0.5, 0.5))
                        famaNum:setColor(cc.c3b(255, 148, 1))
						...
						table.insert(arr,cc.CallFunc:create(function ()
							...
							-- print(num.." k:"..k)
							-- famaNum:setTexture("package_res/games/guangdongjihumj/game/fama/maCount"..num..".png")
						end))
					else
...
function MJFanMa:compose(index)
	...
	-- for _,score in pairs(self.m_data[index].faI9) do
		-- dump(score)
		for k,v in pairs(self.m_data[index].faI9) do
			if v.isM==1 then
				if  keys[v.faI6] then
