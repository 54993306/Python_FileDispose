...
function BranchGameOverLogic:setGameOverDatas(cmd, context)
   ...
   sys:setMjDistrubuteEnd(false)
    ...
    for i=1, playerCount do
        ...
        self.gameOverDatas.score[site].fanma = context.scI[index].fanma or {}
        self.gameOverDatas.score[site].zhongma = context.scI[index].zhongma or {}
        self.gameOverDatas.score[site].buma = context.scI[index].buma or {}
        self.gameOverDatas.score[site].bumaZhongMa = context.scI[index].bumaZhongMa or false
    end

    self.gameOverDatas.maList = context.mas
    self.gameOverDatas.fanMaShow = context.fanma
--    local maList = {}
--    for i,v in pairs(self.gameOverDatas.maList) do
--        v.fanma = context.fanma
--        table.insert(maList,v)
--    end
--    self.gameOverDatas.maList = maList
    self.gameOverDatas.isQuanMa = context.isQuanMa
end
