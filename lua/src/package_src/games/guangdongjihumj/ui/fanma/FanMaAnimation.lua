...
function FanMaAnimation:createBg()
    ...
    for i,v in pairs(data) do
        ...
        if #data > 1 then
            posY = display.cy+((#data/2)-(i-1))*130 - 120
...
function FanMaAnimation:drawFanMaText(bgLayer)
    local fmt = display.newSprite("package_res/games/guangdongjihumj/game/fama/title.png")
    fmt:addTo(bgLayer)
...
function FanMaAnimation:drawHeand(index,bgLayer)
    ...
    local headKuang = display.newSprite("games/common/mj/games/fama/game_player_box.png")
    ...
    Util.updateNickName(name, ToolKit.subUtfStrByCn(strNickName, 0, 6, "..."), 40)
    name:addTo(headBg)
...
function FanMaAnimation:drawBg(index,rsNum)
    ...
    elseif self.m_quanma then
        local quanma = display.newSprite("package_res/games/guangdongjihumj/game/fama/quanma_text.png")
    ...
    for i,v in pairs(mjs) do
        ...
        if rsNum <= 1 then
            if #mjs <= 8 then
                ...
                if #mjs > 12 then
--                    scale = 1-(#mjs)*0.028
                    scale = (display.width/#mjs)/(sprite:getContentSize().width+5)
                else
                    scale = 1-(#mjs-8)*0.08
                end
                sprite:setScale(scale)
                local width = display.cx + ((#mjs+1)/2)*(sprite:getContentSize().width+5)*scale
            ...
            if #mjs < 16 then
                ...
                scale = ((display.width-130)/#mjs)/(sprite:getContentSize().width+5)
                local width = display.cx + ((#mjs+1)/2)*(sprite:getContentSize().width+5)*scale
                posX = 60+width - i*(sprite:getContentSize().width+5)*scale
--                if #mjs >= 20 then
--                    posX = posX + 30
--                end
            end
