...
-- @return void
--]]
function huaijimjGamePlayLayer:onDispenseCard()
    local dispenseData 	= self.gamePlaySystem:getDispenseCardDatas()
    -- Log.i("sunbinLog:dispenseData =============== ", dispenseData)
    ...
    if dispenseData.isBaoma then
    	local baomaList = {}
    	local site = self.gamePlaySystem:getPlayerSiteById(dispenseData.userId)
        self.m_delegate.m_flowerNode:setBuhuaNumber(site, dispenseData.baomaCard)
    end
end

function huaijimjGamePlayLayer:getSelfPlayerPannel()
    return self.playerPannel[enSiteDirection.SITE_MYSELF]
end


function huaijimjGamePlayLayer:runActionAnimation(operateData)
    print("sunbinLog:-----runActionAnimation----------")
...
-- @return void
--]]
function huaijimjGamePlayLayer:getAllPutOutCard()
