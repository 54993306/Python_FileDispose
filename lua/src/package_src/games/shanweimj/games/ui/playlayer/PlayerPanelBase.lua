...
function PlayerPanelBase:playActionAnimation(amimation, site, time, isHide)
    ...
    if amimation ~= "AnimationYANGMA" then
        ...
        ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("package_res/games/shanweimj/extraAnimation.csb")
        armature = ccs.Armature:create("extraAnimation")
        armature:getAnimation():play("AnimationYANGMA")
        armature:performWithDelay(function()        
                MjMediator.getInstance():getEventServer():dispatchCustomEvent(enMjNtfEvent.GAME_ACT_ANIMATE_FINISH_NTF, amimation)
                if isHide then
                    armature:removeFromParent(true)
