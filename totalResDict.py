...
class totalRes:
    ...
    def __init__(self , refresh = True ):
        self.allFiles           = 0  # 记录文件数
        self.filedict           = collections.OrderedDict()  # 文件分类表，有多少中类型资源，每种类型有多少个
        self.notRepeatmd5List   = collections.OrderedDict()  # 存储文件md5值 key:md5 value:path
        self.oldtonewPath       = collections.OrderedDict()  # 新旧路径对应表 key: oldpath ,value : newpath
        self.typeNum            = collections.OrderedDict()  # 存储文件类型和相应的数量
        if refresh:
            self.initDict()
        else:
            self.hasRecordFile()
    ...
    def addToDict(self, filepath):
        md5 = self.createFileMd5(filepath)
        if not md5:
            return False
    ...
    def createFileMd5(self , filepath):
        md5 = comFun.generateFileMd5(filepath)
        self.allFiles += 1
    ...
    # 复制文件
    def copyFile(self):
        comFun.createNewDir(comFun.RESPATH)
        copynum = 0
    ...
    def specialType(self , fileInfo): # or
        ...
        if not filetype:                         # 没有类型的文件不修改文件名
            newpath = comFun.RESPATH + "res_other/" + os.path.basename(path)
    ...
    def getNewFileName(self , filepath):
        ...
        return comFun.RESPATH + outPutPath + "/" + "10" + \
