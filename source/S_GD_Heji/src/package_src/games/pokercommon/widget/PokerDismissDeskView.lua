...
function PokerDismissDeskView:ctor(...)
    -- self.super.ctor(self,"games/common/mj/dismiss_desk2.csb", ...);
    self.super.ctor(self,"package_res/games/pokercommon/dismiss_desk.csb", ...);
...
function PokerDismissDeskView:onInit()
    ...
    local lab_title_0 = ccui.Helper:seekWidgetByName(self.m_pWidget, "lab_title_0");
    lab_title_0:setVisible(false)
...
function PokerDismissDeskView:onClose()
    Log.i("PokerDismissDeskView:onClose()")
...
function PokerDismissDeskView:addPlayers()
    ...
    for i=1, #my_info do
        ...
        local img_frame = ccui.Helper:seekWidgetByName(pan_head, "headImg");
...
function PokerDismissDeskView:onClickButton(pWidget, EventType)
    if EventType == ccui.TouchEventType.ended then
        if pWidget == self.btn_agree then 
            ...
            NativeCall.getInstance():NativeCallUmengEvent(UmengClickEvent.DDZGameAgree)
            Log.i("--wangzhi--UmengClickEvent.DDZGameAgree--")
        ...
        elseif pWidget == self.btn_disagree then
			...
			tmpData.isF=1
			SocketManager.getInstance():send(CODE_TYPE_ROOM, HallSocketCmd.CODE_FRIEND_ROOM_LEAVE,tmpData);
            NativeCall.getInstance():NativeCallUmengEvent(UmengClickEvent.DDZGameDisagree)
            Log.i("--wangzhi--UmengClickEvent.DDZGameDisagree--")
        end
    end
end
...
function PokerDismissDeskView:updateUI(packetInfo)
    ...
    PokerUtils:updateNickName(self.lab_title, string.format("【%s】发起解散牌局申请", nickName), 32)
	...
	elseif(packetInfo.re == 2 )  then --2:不同意
		...
		if packetInfo.usI ~= HallAPI.DataAPI:getUserId() then
           ...
           -- PokerUtils:updateNickName(content, data.content, 32)
		end
	end
end
