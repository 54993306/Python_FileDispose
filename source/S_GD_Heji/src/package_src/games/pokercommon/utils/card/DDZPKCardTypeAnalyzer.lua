...
function DDZPKCardTypeAnalyzer.getCardType(cards)
	if not cards or #cards == 0 then
		return DDZPKCardTypeAnalyzer.FuncTypeMap[DDZPKCardTypeAnalyzer.isError]
	end

	local n = #cards --牌数量
    table.sort(cards) --牌从小到大排序

	local isThisType
	local keyCard
	if n >= 5 then --牌数量大于等于5张
		isThisType, keyCard = DDZPKCardTypeAnalyzer.isOneLine(cards)--判断该牌型是否是顺子
		Log.i("--wangzhi--判断是否是顺子--",isThisType, keyCard)
		Log.i("DDZPKCardTypeAnalyzer.getCardType", isThisType, keyCard)
		if isThisType then --判断该牌型是否是存在
			return DDZPKCardTypeAnalyzer.FuncTypeMap[DDZPKCardTypeAnalyzer.isOneLine], keyCard
		end
	end

	-- if n >= 8 then
	-- 	if DDZPKCardTypeAnalyzer.isDoubleFourLine(cards) then
	-- 		return DDZPKCardTypeAnalyzer.FuncTypeMap[DDZPKCardTypeAnalyzer.isError]
	-- 	end
	-- end
	if n <= 8 then --判断牌的数量小于等于8张
		for k, v in pairs(DDZPKCardTypeAnalyzer.NumFuncMap[n]) do
			isThisType, keyCard = v(cards)
			if isThisType then
				return DDZPKCardTypeAnalyzer.FuncTypeMap[v], keyCard
			end
		end
	else
		Log.i("DDZPKCardTypeAnalyzer.getCardType > 8")
		for i = 2, 5 do
		Log.i("DDZPKCardTypeAnalyzer.getCardType 2-5",i)
			if n%i == 0 then
				local index = i*5
				Log.i("DDZPKCardTypeAnalyzer.getCardType index", index)
				for k, v in pairs(DDZPKCardTypeAnalyzer.NumFuncMap[index]) do
					isThisType, keyCard,isThreeLineType = v(cards)
					Log.i("DDZPKCardTypeAnalyzer.getCardType sss", isThisType, keyCard)
					if isThisType then
						Log.i("--wangzhi--能够判断出结果的时候,key是多少",k,v,keyCard)
						if DDZPKCardTypeAnalyzer.FuncTypeMap[v] == 6 and isThreeLineType ~= nil then
							return isThreeLineType,keyCard
						end
						return DDZPKCardTypeAnalyzer.FuncTypeMap[v], keyCard
					end
				end
	...
	Log.i("DDZPKCardTypeAnalyzer.getCardType last")
	return DDZPKCardTypeAnalyzer.FuncTypeMap[DDZPKCardTypeAnalyzer.isError]
end
...
-- @pram cards:牌组
--------------------------------------------------------------------------
function DDZPKCardTypeAnalyzer.isOneLine(cards)
	return DDZPKCardTypeAnalyzer.isLine(cards), cards[1]
end
