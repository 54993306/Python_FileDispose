...
--            drawLine    = true,             --是否在下面绘制一行线
--        },
--    isDrawLine = true,                      --是否绘制横分割(true为绘制)
--    }
--}

local labelOffect_x = 1                         --label的X偏移量
local parenthesesOffectX = 65                   --第二个括号的偏移量
local labelGreyColor = cc.c3b(105,105,105)      --变灰后的label颜色
local normalCheckColor =  cc.c3b(255, 255, 255)
local lineImage = "hall/Common/line2.png"       --底线的路径
local roundCheckBox = "games/common/game/roundSelectCheckBox.csb"
local selectCheckBox = "games/common/game/selectCheckBox.csb"
local controlsName = {
        label = "Label_name",
        check = "CheckBox",
      }
...
function CommonCheckBoxPanel:createData(data)
    self.m_itemMinimum          = 3                                         --单行最小项数
    self.m_itemBiggest          = 5                                         --单行最大项数
    ...
    self.m_startCBPanelX        = G_ROOM_INFO_FORMAT.firstPosX or 110       -- checkBox的起始x位置
    if IsPortrait then
	    self.m_panelHeight          = G_ROOM_INFO_FORMAT.lineHeight or 80       -- 层高度
	    self.m_panelWidth           = G_ROOM_INFO_FORMAT.lineWidth - self.m_startCBPanelX - 0 or 720       -- 层宽度
	    self.m_panelOffsetY         = 8                                         --Y的偏移量
    else
	    self.m_panelHeight          = G_ROOM_INFO_FORMAT.lineHeight or 60       -- 层高度
	    self.m_panelWidth           = G_ROOM_INFO_FORMAT.lineWidth - self.m_startCBPanelX - 38 or 720       -- 层宽度
	    self.m_panelOffsetY         = -5                                         --Y的偏移量    
    end

    self.m_itemTitle            = -55                                       --子项title的偏移量
    self.m_itemScale            = 0.8                                       --子项的缩放比例
    self.m_itemOffectY          = 5                                        --子项的Y偏移量

    

    self.m_titleStartX          = G_ROOM_INFO_FORMAT.titlePosX or 20        --title的起始偏移位置

    self.m_TitleFontColor       = G_ROOM_INFO_FORMAT.titleFontColor or cc.c3b(67, 67, 67)  --标题默认颜色
    self.m_TitleFontSize        = G_ROOM_INFO_FORMAT.titleFontSize or 28    --标题默认大小

    self.m_itemPanelWidth       = 160                                       -- 子项层宽
    self.m_itemChildWidth       = 240                                       -- 子项离母项的宽度

    self.m_FontName             = "hall/font/fangzhengcuyuan.TTF"           -- 文字字体
	self.m_FontColorMoren       = G_ROOM_INFO_FORMAT.normalColor or  cc.c3b(67, 67, 67) -- 文字默认颜色
...
function CommonCheckBoxPanel:createCheckPanel()
    ...
    self.roundSelectPanel = ccs.GUIReader:getInstance():widgetFromBinaryFile(roundCheckBox)
    self.roundSelectPanel:setVisible(false)
    self.roundSelectPanel:setSwallowTouches(false)
    self.roundSelectPanel:setPositionY(self.roundSelectPanel:getPositionY()+25)
    local label_name = ccui.Helper:seekWidgetByName(self.roundSelectPanel,controlsName.label)
    label_name:setColor(self.m_FontColorMoren)
	label_name:setFontName(self.m_FontName)
    label_name:setFontSize(self.m_textSize)
    label_name:setPosition(cc.p(label_name:getPositionX()-labelOffect_x,label_name:getPositionY()))

    self.selectPanel = ccs.GUIReader:getInstance():widgetFromBinaryFile(selectCheckBox)
    self.selectPanel:setVisible(false)
    self.selectPanel:setSwallowTouches(false)
    local label_name = ccui.Helper:seekWidgetByName(self.selectPanel,controlsName.label)
    label_name:setColor(self.m_FontColorMoren)
	label_name:setFontName(self.m_FontName)
    label_name:setFontSize(self.m_textSize)
    label_name:setPosition(cc.p(label_name:getPositionX()-labelOffect_x,label_name:getPositionY()))
...
function CommonCheckBoxPanel:drawCheckBox()
    ...
    for i,v in pairs(self.m_data) do
        ...
        for j,k in pairs(v) do
            local num = #v
            if (k.isLink or not isHuan) and i > 1 then
                num = #self.m_data[i-1]
            ...
            local posX,posY = self:getSinglePos(j,i+itemNum,num,title,k)
            local panel = self:drawSelectPanel(posX,posY,k)
            local itemPanel = nil
            local isLine = false
            if type(k.itemSelect) == "table" then
                isLine,itemPanel = self:drawItemPanel(posX,posY,k.itemSelect, k.isSelect)
...
--计算选项层级的位置信息
function CommonCheckBoxPanel:getSinglePos(x,y,num,title,data)
...
--绘制选项层
function CommonCheckBoxPanel:drawSelectPanel(posX,posY,data,isItem, panelIsSelect)
    local panel = nil
    if data.multi then
    ...
    local label_name = ccui.Helper:seekWidgetByName(panel,controlsName.label)
    local name = data.name
    if data.title then
        name = name ..":"
    end
    label_name:setString(name)
    local checkBox = ccui.Helper:seekWidgetByName(panel,controlsName.check)
    ...
    if data.drawLine then
        self:drawNewLine(panel)
    end
    panel:addTo(self)
    return panel
end
function CommonCheckBoxPanel:drawNewLine(panel)
    local line = display.newScale9Sprite(lineImage,G_ROOM_INFO_FORMAT.lineWidth/2,panel:getPositionY(),self.lineSize)
	line:addTo(self)
end
--当为子选项时绘制
function CommonCheckBoxPanel:drawItemPanel(x,y,data, panelIsSelect)
    local isLine = false
    local itemPanel = {}
    for i,v in pairs(data) do
        ...
        local panel = self:drawSelectPanel(posX,posY,v,not isLine, panelIsSelect)
        table.insert(itemPanel,panel)
        if not isLine and #data > 1 then
...
function CommonCheckBoxPanel:drawItemArea(posX,posY,index,panel)
    ...
    if index == 1 then
        ...
        title:setString(")")
        local label = ccui.Helper:seekWidgetByName(panel,controlsName.label)
        labelSize = label:getContentSize()
        title:setPosition(cc.p(labelSize.width + parenthesesOffectX,self.m_textSize+self.m_itemOffectY))
    end
    title:addTo(panel)
end
--如有换行子项时整体向上移动子项个高度
...
function CommonCheckBoxPanel:drawTitle()
	...
	title:setString(self.m_titleText)
    local titleSize = title:getContentSize()
    local cbPSize = self.roundSelectPanel:getContentSize()
    title:setPosition(cc.p(titleSize.width/2+self.m_titleStartX,(#self.m_data)*self.m_panelHeight-titleSize.height + self.m_panelOffsetY/2))
    title:addTo(self)
end
function CommonCheckBoxPanel:checkBoxSelect()
    for i,v in pairs(self.m_CommonCheckBoxPanel) do
        for j ,k in pairs(v.panel) do
            local checkBox = ccui.Helper:seekWidgetByName(k,controlsName.check)
            checkBox:addTouchEventListener(function(obj, event)
                ...
                elseif event == ccui.CheckBoxEventType.unselected or event == ccui.TouchEventType.canceled then
                end
            end)
            if v.itemSelect[j] ~= nil then
                -- local parentSelected = checkBox:isSelected()
                for m ,n in pairs(v.itemSelect[j]) do
                    local checkBox = ccui.Helper:seekWidgetByName(n,controlsName.check)
                    checkBox:addTouchEventListener(function(obj, event)
                        local selected = ccui.CheckBoxEventType.selected
                    ...
                    -- checkBox:setTouchEnabled(parentSelected)
                end
            end
        end
...
function CommonCheckBoxPanel:drawLine()
    ...
    local line = display.newScale9Sprite(lineImage,G_ROOM_INFO_FORMAT.lineWidth/2,0,self.lineSize)
	line:addTo(self)
end
...
function CommonCheckBoxPanel:setCheckBoxSelect(group,index,pIndex,table)
    local CommonCheckBoxPanel = ccui.Helper:seekWidgetByName(group[index],controlsName.check)
    for i,v in pairs(group) do
        local item = table.data[index]
        local checkBox = ccui.Helper:seekWidgetByName(v,controlsName.check)
        local label_name = ccui.Helper:seekWidgetByName(v,controlsName.label)
        if item.multi then
            if i ~= index then
            ...
            checkBox:setTouchEnabled(true)
            self:setCbSelectItemPanel(table,index,i,checkBox)
        else
            if i == index then
                checkBox:setSelected(true)
                if not table.data or not table.data[i] or not table.data[i].title then
...
function CommonCheckBoxPanel:setCbSelectItemPanel(table,index,pIndex,cbPanel,items)
    if table == nil or index ~= pIndex or #table.itemSelect <= 0 then
        return
    end
    ...
    if isSelect then
        for i,v in pairs(table.itemSelect[index]) do
            local checkBox = ccui.Helper:seekWidgetByName(v,controlsName.check)
            local label_name = ccui.Helper:seekWidgetByName(v,controlsName.label)
            checkBox:setSelected(false)
            label_name:setColor(self.m_FontColorMoren)
        ...
        for i,v in pairs(table.itemSelect[index]) do
            ...
            if i == 1 then
                checkBox:setSelected(true)
                label_name:setColor(self.m_selectFontColor)
                if not table.data[index].itemSelect[i].multi then
                    checkBox:setTouchEnabled(false)
                else
                    checkBox:setTouchEnabled(true)
                end
...
function CommonCheckBoxPanel:setCbItemPanel(data,index,pIndex,cbPanel)
    if #data.itemSelect > 0 and data.itemSelect[pIndex] ~= nil then
        if pIndex == index then
            ...
            self:setCbSelectItemPanel(data,index,pIndex,cbPanel,true)
        else
        ...
        for j,k in pairs(v.panel) do
            ...
            if checkBox:isSelected() then
                ...
                if v.itemSelect[j] ~= nil and v.itemSelect[j][1] ~= nil then
                    ...
                    for m,n in pairs(v.itemSelect[j]) do
                        itemSelect = n
                        local checkBox = ccui.Helper:seekWidgetByName(itemSelect,controlsName.check)
                        if checkBox:isSelected() then
                            wanfa = wanfa.."_"..v.data[j].itemSelect[m].chick
                        end
                    end
                    break
                end
...
function CommonCheckBoxPanel:getSelectedInfo()
    local selectedInfo = {}
    for i,v in pairs(self.m_CommonCheckBoxPanel) do
        for j,k in pairs(v.panel) do
            local checkBox = ccui.Helper:seekWidgetByName(k,"CheckBox")
            selectedInfo[#selectedInfo + 1] = checkBox:isSelected()
            if v.itemSelect[j] ~= nil and v.itemSelect[j][1] ~= nil then
                for m,n in pairs(v.itemSelect[j]) do
