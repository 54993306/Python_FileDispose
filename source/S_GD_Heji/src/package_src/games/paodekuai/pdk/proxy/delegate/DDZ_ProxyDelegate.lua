...
function DDZ_ProxyDelegate:ReadServerStructInfor(eventdata)
    ...
    elseif eventdata.cmd == DDZSocketCmd.CODE_REC_TOTAL_GAME_OVER then
        ...
        -- kSystemConfig:cacheEnterData("active" , true)
        -- 下面的改到了ddzroom单局结算的地方去处理
        -- HallAPI.EventAPI:dispatchEvent(PokerEventDef.GameEvent.GAME_REQ_SHOWTOTALOVER)
        --HallAPI.EventAPI:dispatchEvent(POKERCONST_EVENT_NETDISPATCH, true)
    elseif eventdata.cmd == DDZSocketCmd.CODE_REC_ACITVITY_JIESUAN then
        self:onRecvActivityJieSuan(eventdata.packetInfo)
    -- 退出房间
    ...
    -- 收到服务器下发的重连消息
    elseif eventdata.cmd == DDZSocketCmd.CODE_PLAYER_ROOM_STATE then
...
--玩家解散
function DDZ_ProxyDelegate:FriendRoomExit(packetInfo)
    -- 收到服务器的消息,退出到大厅
...
function DDZ_ProxyDelegate:onRecvTotalGameOver(packetInfo)
    ...
    --缓存总结算消息
    local tmpData =  DataMgr.getInstance():getObjectByKey(DDZDataConst.DataMgrKey_FRIENDTOTALDATA) or {}
...
function DDZ_ProxyDelegate:onRecvActivityJieSuan(packetInfo)
    local tmpData = DataMgr.getInstance():getObjectByKey(DDZDataConst.DataMgrKey_FRIENDTOTALDATA) or {}
    tmpData.activityList = packetInfo
    DataMgr.getInstance():setObject(DDZDataConst.DataMgrKey_FRIENDTOTALDATA, tmpData)
end
...
function DDZ_ProxyDelegate:onRecvFriendRoomInfo(packetInfo)
    Log.d("DDZ_ProxyDelegate:onRecvFriendRoomInfo", packetInfo)
