...
--
local PokerRoomPlayerInfoView = require("package_src.games.pokercommon.widget.PokerRoomPlayerInfoView")
...
function DDZPlayerView:updatePlayerInfo()
    ...
    Log.i("head url ", url)
    self:ipXiangTong(player,self.img_head)
...
--检测ip是否相同
function DDZPlayerView:ipXiangTong(playerModel, head)
    -- local playerInfos = kFriendRoomInfo:getRoomInfo();
    local players = DataMgr:getInstance():getTableByKey(DDZDataConst.DataMgrKey_PLAYERLIST);
    local myIp = playerModel:getProp(DDZDefine.IP);
    local ipA = {};
    local player = 0;
    local isIpHand = false;
    for i ,v in pairs(players) do
        if v ~= nil and playerModel:getProp(DDZDefine.USERID) ~=v:getProp(DDZDefine.USERID) then
            if v ~= nil then
                if myIp == v:getProp(DDZDefine.IP) then
...
--绘制ip相同
function DDZPlayerView:drawIpXiangTong(head)
    local headOneIp = head:getChildByName("ipxiangtong")
    if headOneIp == nil then
...
function DDZPlayerView:onClickButton(pWidget, EventType)
    if EventType == ccui.TouchEventType.ended then
        NativeCall.getInstance():NativeCallUmengEvent(UmengClickEvent.DDZGamePlayerHead)
