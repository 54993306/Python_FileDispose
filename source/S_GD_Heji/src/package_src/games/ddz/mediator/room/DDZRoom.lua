...
function DDZRoom:onUpdateSignal(event)
    ...
    Log.i("DDZRoom:onUpdateSignal info", info)
...
function DDZRoom:refreshSignalAndBattery(refreshSignal)
    Log.i("DDZRoom:refreshSignalAndBattery", "self.m_showSignal:", self.m_showSignal, "isWifi():", HallAPI.DataAPI:isWifi(), "refreshSignal:", refreshSignal)
    ...
    if refreshSignal then
        ...
        Log.i("DDZRoom:refreshSignalAndBattery", "rssi:", rssi)
        if rssi >= 4 then
...
function DDZRoom:onClickButton(pWidget, EventType)
    if EventType == ccui.TouchEventType.ended then
        ...
        elseif pWidget == self.btn_menu then
            ...
            NativeCall.getInstance():NativeCallUmengEvent(UmengClickEvent.DDZMoreButton)            
        ...
        elseif pWidget == self.btn_chat then
            ...
            NativeCall.getInstance():NativeCallUmengEvent(UmengClickEvent.DDZGameChatText)
        ...
        elseif pWidget == self.btn_jiesan then
            NativeCall.getInstance():NativeCallUmengEvent(UmengClickEvent.DDZGameDissmissRoom)
            self:jiesanBtnEvent()
...
function DDZRoom:onTouchSayButton(pWidget, EventType)
    ...
    if EventType == ccui.TouchEventType.began then
        NativeCall.getInstance():NativeCallUmengEvent(UmengClickEvent.DDZGameVoiceInput)
