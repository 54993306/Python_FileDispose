...
local PokerCommonSocketCmd = require("package_src.games.guandan.gdcommon.data.PokerCommonSocketCmd")
...
local PokerDismissDeskView = require("package_src.games.guandan.gdcommon.widget.PokerDismissDeskView")
...
function PokerRoomBase:ctor(...)
    ...
    self.m_netDialogYesCallback = function ()
        ...
        SocketManager.getInstance():addGDCloseSocket(false)
   ...
   local data = {}
    ...
    --[[
        ...
        if self._selectBtn.agree == false then
            ...
            NativeCallUmengEvent(UmengClickEvent.GDGameAskDismiss)
    ...
    data.cancelCallback = function()
        NativeCallUmengEvent(UmengClickEvent.GDGameContinue)
    end
...
function PokerRoomBase:onNetWorkReconnected()
    ...
    if kLoginInfo:requestLogin() then
        self.m_pWidget:performWithDelay(function()
            ...
            PokerToast.getInstance():show("重连成功");
...
function PokerRoomBase:showDialogReturnToLogin(content)
    HallAPI.ViewAPI:hideLoadingView()
    HallAPI.ViewAPI:hideLoadingView("networkState")
    SocketManager.getInstance():addGDCloseSocket(true)
...
function PokerRoomBase:showDialogRetryConnect(content)
    ...
    data.yesTitlePath = "package_res/games/guandan/image/image_reconnect.png"
    data.yesTitlePathType = ccui.TextureResType.localType

    data.cancelCallback = self.m_netDialogCancalCallback
    data.cancelTitlePath = "package_res/games/guandan/image/image_quit.png"
