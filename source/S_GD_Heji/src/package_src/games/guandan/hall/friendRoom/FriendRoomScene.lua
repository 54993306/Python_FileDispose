...
function FriendRoomScene:ctor(data)
    ...
    self.m_listeners = {}
    self:registMsgs()
    cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/guandan/pokercommon.plist","package_res/games/guandan/pokercommon.png")
    cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/guandan/guandan.plist","package_res/games/guandan/guandan.png")
    self.super.ctor(self.super,"package_res/games/guandan/hall/friendRoomScene_gd.csb", data)
...
function FriendRoomScene:onInit()
    ...
    local selectRoomInfo = kFriendRoomInfo:getSelectRoomInfo()
    --队友组队
    self.isOrganizeTeam = false
    if string.find(selectRoomInfo.wa,"duiyouzudui") then
        self.isOrganizeTeam = true
    end
    local isUpdateGrade = false
    if string.find(selectRoomInfo.wa,"bushengji") then
    else
        isUpdateGrade = true
    end
    ...
    --准备按钮
    self.btnReady = ccui.Helper:seekWidgetByName(self.m_pWidget, "btn_ready")
    ...
    ui:setVisible(false)
    self.moreLayer = DDZMoreLayer.new(ui, {isGuandan = true})
    self.lab_roomId = ccui.Helper:seekWidgetByName(self.m_pWidget,"lab_roomid")
    self.lab_roomId:setString(string.format("房间号:%d",self.m_roomInfo.pa))
    local text_room_id = ccui.Helper:seekWidgetByName(self.m_pWidget,"text_room_id")
    text_room_id:setString(string.format("房间号:%d",self.m_roomInfo.pa))

    local str = ""
    if self.m_roomInfo.RoJST == 0  then
        str = "亲友圈付费"
    elseif self.m_roomInfo.RoJST == 1  then
        str = "房主付费"
    elseif self.m_roomInfo.RoJST == 2  then
        str = "大赢家付费"
    elseif self.m_roomInfo.RoJST == 3 then
        str = string.format("AA付费(每人%s钻石)",math.ceil( self.m_roomInfo.RoFS / self.m_roomInfo.plS ))
    end
    local ruleStrRet = kFriendRoomInfo:getRuleStrRet(kRuleWidth, kRuleFontSize)
    if ruleStrRet.rows > 0 then
        str = str .. ruleStrRet.ruleStr
    end
    if isUpdateGrade then
    else
        local juStr = string.format(" %s局", selectRoomInfo.roS)
        str = str .. juStr
    end

    local txt_room_rule = ccui.Helper:seekWidgetByName(self.m_pWidget,"txt_room_rule")
    txt_room_rule:setString(str)
    ...
    for i = 1, #personSites do
        local strName = "pan_head".. personSites[i]
        ...
        self.m_playerHeadList[i]:setBtnExchange(self.isOrganizeTeam)
    ...
    local copyBtnLayout= ccui.Helper:seekWidgetByName(self.m_pWidget, "panel_copy")
    ...
    self.chatNode = {}
    for i=1, 4 do
        self.chatNode[i] = ccui.Helper:seekWidgetByName(self.m_pWidget, "panel_chat_"..i)
    end

    if self.m_roomInfo.ExR == 2 then
        ...
        for i,v in pairs(self.playerPanel) do
            if chatData.usI == self:getPlayerId(i) then
                if chatData.ty == 0 then
                    ...
                    local chat_bg = self.chatNode[site]:getChildByName("img_pos")
                    if face == "face_" then
                        ...
                        if tonumber(index) > 0 and tonumber(index) <= 24 then
                            local playerChat = PlayerChat.new(site, self.chatNode[site], 2, chatData, chat_bg)
                        else
                            local playerChat = PlayerChat.new(site, self.chatNode[site], 1, chatData, chat_bg)
                        end
                            ...
                            if _gameChatTxtCfg[index] then
                                ...
                                for k, v in pairs(playerInfos.pl) do
                                    if v.usI == chatData.usI then
                                        SoundManager.playEffect(_getGameLiaotianduanyuKey2(v.se or 2, index))
                                        break
                                    end
                                end
                            end
    ...
    for i = 1, #self.m_playerHeadList do
        ...
        if playerInfo then
            ...
            if playerInfo.usI == kUserInfo:getUserId() then
                if playerInfo.isR == 1 then
                    self.btnReady:loadTextureNormal("package_res/games/guandan/hall/huanpi2/friendroomscene/btn_readyCancel.png")
                else
                    self.btnReady:loadTextureNormal("package_res/games/guandan/hall/huanpi2/friendroomscene/btn_getReady.png")
                end

                self.isReady = playerInfo.isR
                self.m_playerHeadList[i]:setBtnExchange(false)
            else
                self.m_playerHeadList[i]:setBtnExchange(self.isOrganizeTeam)
            end
