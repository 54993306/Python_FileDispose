...
local CardSet = class("package_src.games.guandan.gdpoker.CardSet")
local CARD = require("package_src.games.guandan.gdpoker.Card")

PokerUtil = require("package_src.games.guandan.gdpoker.PokerUtil")
...
function CardSet:AddCard(card)
	...
	if (self.m_nCurrentLength < MAX_CARDS_LEN) then
		if ((card.shape >= enmCardShape.ECS_SHAPE_DIAMONDS and card.shape <= enmCardShape.ECS_SHAPE_JOKER
             and card.number >= enmCardNumber.ECN_NUM_3 and card.number <= enmCardNumber.ECN_NUM_JOKER)
...
--ghost：		鬼牌集合
--返回值：		无
function CardSet:setCardGhost(ghost)
	if ghost and table.nums(ghost) then
		self.m_ghostCard = ghost
	end
	for i,v in pairs(self.m_Cards) do
		for j,gv in pairs(ghost) do
			if v.originalVal == gv then
				v:setGhost()
			end
		end
	end
...
--函数功能：	添加万能牌
function CardSet:addGhostCards(card)
	table.insert(self.m_ghostCard,card)
end

--函数功能：	检索牌里面的癞子
--返回值：		万能牌
function CardSet:getGhostCards()
	return self.m_ghostCard
end

--函数功能：	检索牌里万能牌的个数
function CardSet:getGhostLength()
	return table.nums(self:getGhostCards())
end
...
--函数功能：	根据牌值number排序函数
--mod:		   排序模式
--				(取值为0时，按权值number(考虑花色)升序排列)
--				(取值为1时，按权值number(考虑花色)降序排列)
function CardSet:SortByNumber(mod)
	mod = mod or 0
	if 0 == mod then
