...
local GDTopHandCardView = class("GDTopHandCardView", function ()
    return display.newLayer();
end);

local HAND_LAYER_POSITIONX = -480
local HAND_LAYER_POSITIONY = 70
...
--返回值：      无
--widget：      ui
function GDTopHandCardView:ctor( widget)
...
--函数功能：    析构函数
--返回值：      无
function GDTopHandCardView:dtor()
...
--函数功能：    ui关闭函数
--返回值：      无
function GDTopHandCardView:close()
    for _,v in pairs(self.listeners) do
        HallAPI.EventAPI:removeEvent(v)
    end
    self.listeners = nil
...
function GDTopHandCardView:dealCard(cards, isReconnect,seat)
    -- 这里只是为了处理战绩回放,重连不处理,避免对局中有重连，导致牌局异常
    if isReconnect then
        return
    end

    if seat and seat ~= GDConst.SEAT_TOP then
        return
    end

    Log.i("--wangzhi--GDTopHandCardView--",cards)
...
--函数功能：    根据牌值手牌排序
--返回值：      无
function GDTopHandCardView:sortByValue()
...
--返回值:         无
--isUpateAll ：   是否更新所有牌
function GDTopHandCardView:updatePoker()
...
--函数功能：      重设牌位置
--返回值:         无
function GDTopHandCardView:resetPosition()
    ...
    for i,v in ipairs(self.m_cards) do
        ...
        if i>14 then
            self.m_cards[i]:setPositionY(HAND_LAYER_POSITIONY-110)
        else
            self.m_cards[i]:setPositionY(HAND_LAYER_POSITIONY-60)
        end
...
--cards：       要删除手牌
--seat：        删除牌的座位
function GDTopHandCardView:onDelCard(cards, seat)
