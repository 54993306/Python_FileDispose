...
local GDLeftHandCardView = class("GDLeftHandCardView", function ()
...
--返回值：      无
--widget：      ui
function GDLeftHandCardView:ctor( widget)
...
--函数功能：    析构函数
--返回值：      无
function GDLeftHandCardView:dtor()
...
--函数功能：    ui关闭函数
--返回值：      无
function GDLeftHandCardView:close()
    for _,v in pairs(self.listeners) do
        HallAPI.EventAPI:removeEvent(v)
    end
    self.listeners = nil
...
function GDLeftHandCardView:dealCard(cards, isReconnect,seat)
    -- 这里只是为了处理战绩回放,重连不处理,避免对局中有重连，导致牌局异常
    if isReconnect then
        return
    end
    -- 不传座位或者
    if seat and seat ~= GDConst.SEAT_LEFT then
        return
    end

    Log.i("--wangzhi--GDLeftHandCardView--",cards)
...
--函数功能：    根据牌值手牌排序
--返回值：      无
function GDLeftHandCardView:sortByValue()
...
--返回值:         无
--isUpateAll ：   是否更新所有牌
function GDLeftHandCardView:updatePoker()
...
--函数功能：      重设牌位置
--返回值:         无
function GDLeftHandCardView:resetPosition()
...
--cards：       要删除手牌
--seat：        删除牌的座位
function GDLeftHandCardView:onDelCard(cards, seat)
    Log.i("--wangzhi--GDLeftHandCardView--onDelCard--",cards,seat)
    if seat ~= GDConst.SEAT_LEFT  then
        return
    end
    -- Log.i("--wangzhi--GDLeftHandCardView--",cards,seat)
