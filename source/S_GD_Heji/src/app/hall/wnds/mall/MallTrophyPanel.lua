...
fun.initTrophyItem = function(self, item,index)
    ...
    if IsPortrait then -- TODO
        ...
        local showData = json.decode(name_data) or {}
        local contF = showData.title
        local contL = showData.content
        if contF ~= nil and contL ~= nil then
            name:setString(contF)
            trophyName:setString(contL)
        else
...
fun.secondAffirm = function(pData)
    ...
    local ConversionDialog = require("app.hall.wnds.mall.ConversionDialog")
    UIManager.getInstance():pushWnd(ConversionDialog, data)
end
