...
--type = 2,code=22008, 私有房准备
--##  userId  玩家ID
--##  re  int  结果（-1 失败，1 成功）
function FriendRoomSocketProcesser.sendRoomReady()
    local data = {
        userId = kUserInfo:getUserId()
    }
    SocketManager.getInstance():send(CODE_TYPE_ROOM, HallSocketCmd.CODE_FRIEND_ROOM_READY, data)
end
  

--type = 2,code=20015, 私有房玩家座位交换问询   client  <--> server
--##  asUI       发起问询的用户ID 发起人
--##  beAUI     收到请求的玩家Id
...
function FriendRoomSocketProcesser.sendPrivateRoomSeats(tmpData)
   SocketManager.getInstance():send(CODE_TYPE_ROOM, HallSocketCmd.CODE_PRIVATE_ROOM_SEATS_REQ, tmpData)
end
...
function FriendRoomSocketProcesser.sendRoomEnter(tmpData)
  if table.nums(tmpData) == 1 then
    if tonumber(tmpData.pa) == 0 then
...
function FriendRoomSocketProcesser:recvFriendRoomExit(cmd,packetInfo)
    Log.d("收到服务端的牌桌关闭",packetInfo)
  packetInfo = checktable(packetInfo)
  self.m_delegate:handleSocketCmd(cmd,packetInfo);
end
...
function FriendRoomSocketProcesser:repActivityJiesuan(cmd, packetInfo)
    -- body
    Log.i("MjGameSocketProcesser:repActivityJiesuan............", packetInfo)
    packetInfo = checktable(packetInfo)
	self.m_delegate:handleSocketCmd(cmd,packetInfo);

end

function FriendRoomSocketProcesser:recvPrivateRoomSeats(cmd,packetInfo)
  packetInfo = checktable(packetInfo)
  self.m_delegate:handleSocketCmd(cmd,packetInfo);
end

function FriendRoomSocketProcesser:recClubModelCreate(cmd, packetInfo)
    -- body
    -- Log.i("MjGameSocketProcesser:repActivityJiesuan............", packetInfo)
    packetInfo = checktable(packetInfo)
	self.m_delegate:handleSocketCmd(cmd,packetInfo);
...
[HallSocketCmd.CODE_PRIVATE_ROOM_SEATS_REQ] = FriendRoomSocketProcesser.recvPrivateRoomSeats;
...
[HallSocketCmd.CODE_PLAYER_ROOM_STATE] = FriendRoomSocketProcesser.recvFriendRoomExit;--解散桌子信息
...
[HallSocketCmd.CODE_REC_ACITVITY_JIESUAN] = FriendRoomSocketProcesser.repActivityJiesuan;

[HallSocketCmd.CODE_FRIEND_ROOM_CREATE] = FriendRoomSocketProcesser.recvRoomCreate;  --InviteRoomCreate   创建邀请房结果
