...
function FriendRoomEnterInfo:updateUI()
  ...
  --局数
    ...
    if roomInfo.RoJST == 3 then
        ...
        if tonumber(tmpData.gaI) ==20011 then
            if (tonumber(tmpData.roS) == 40 or tonumber(tmpData.roS) == 4) and string.find(tmpData.wa,"|shengji") then
                sushouLabel:setString(string.format("过6(每人%s钻)",zhuanshi))
            elseif (tonumber(tmpData.roS) == 80 or tonumber(tmpData.roS) == 8) and string.find(tmpData.wa,"|shengji")  then
                sushouLabel:setString(string.format("过10(每人%s钻)",zhuanshi))
            elseif (tonumber(tmpData.roS) ==120 or tonumber(tmpData.roS) == 12) and string.find(tmpData.wa,"|shengji")  then
                sushouLabel:setString(string.format("过A(每人%s钻)",zhuanshi))
            end
        end
    else
        sushouLabel:setString(string.format("%s局(%s钻)",tmpData.roS,tmpData.RoFS))
          if tonumber(tmpData.gaI) ==20011 then
            if (tonumber(tmpData.roS) == 40 or tonumber(tmpData.roS) == 4) and string.find(tmpData.wa,"|shengji")  then
                sushouLabel:setString(string.format("过6(%s钻)",tmpData.RoFS))
            elseif (tonumber(tmpData.roS) == 80 or tonumber(tmpData.roS) == 8) and string.find(tmpData.wa,"|shengji")  then
                sushouLabel:setString(string.format("过10(%s钻)",tmpData.RoFS))
            elseif (tonumber(tmpData.roS) ==120 or tonumber(tmpData.roS) == 12) and string.find(tmpData.wa,"|shengji")  then
                sushouLabel:setString(string.format("过A(%s钻)",tmpData.RoFS))
            end
        end
    end
