...
function OpenRoomGame:gameOverUICallBack(isMaintain)
    ...
    if self.m_socketInfo == nil then
        ...
        self:OnRecvRoomReWard(self.m_socketInfo)
        self.m_socketInfo = nil;
    end
end
...
function OpenRoomGame:recvRoomReWard(packetInfo)
    ...
    --缓存总结算消息
	self.m_socketInfo = self.m_socketInfo or {}
    self.m_socketInfo.param = packetInfo
end
--添加总成绩掉落获取
function OpenRoomGame:repActivityJiesuan( packetInfo)
    self.m_socketInfo = self.m_socketInfo or {}
    self.m_socketInfo.activityList = packetInfo
end
...
--玩家解散
function OpenRoomGame:recvFriendRoomExit(packetInfo)
    -- 收到服务器的消息,退出到大厅
...
function OpenRoomGame:onNetWorkReconnected()
    ...
    if UIManager:getInstance():getWnd(FriendOverView) or UIManager:getInstance():getWnd(FriendTotalOverView) then
...
OpenRoomGame.s_socketCmdFuncMap = {
    ...
    [HallSocketCmd.CODE_PLAYER_ROOM_STATE] = OpenRoomGame.recvFriendRoomExit; --私有房间状态

    [HallSocketCmd.CODE_REC_ACITVITY_JIESUAN] = OpenRoomGame.repActivityJiesuan
};
