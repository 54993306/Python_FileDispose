...
function ToolKit.getHexStrFromStr(strContent)
    local tab = {}
    local str = ""
    for i = 1, #strContent do
        -- Log.i(string.byte(strContent, i))
        str = str .. string.format("%02X", string.byte(strContent, i))
        -- table.insert(tab, string.byte(strContent, i))
    end
    -- Log.i("ToolKit.printStrToHex length : " .. #strContent .. " content: " .. str)
    return str
end

function ToolKit.getHexStrFromBytes(bytesTab)
    local str = ""
    for i = 1, #bytesTab do
        -- Log.i(string.byte(strContent, i))
        str = str .. string.format("%02X", bytesTab[i])
        -- table.insert(tab, string.byte(strContent, i))
    end
    -- Log.i("ToolKit.printBytesToHex length : " .. #bytesTab .. " content: " .. str)
    return str
end

function ToolKit.getStrFromHexStr(hexStr)
    local res = ""
    if (#hexStr % 2 ~= 0) then
        Log.e("需要输入偶数位的16进制字符串")
        return res
    end
    local len = #hexStr / 2
    for i = 1, len do
        res = res .. string.char(tonumber(string.sub(hexStr, i*2 - 1, i*2), 16))
    end
    return res
end

function ToolKit.getStrFromBytes(bytesTab)
    local res = ""
    for i = 1, #bytesTab do
        res = res .. string.char(bytesTab[i])
    end
    return res
end
