...
-- 获取分享防屏蔽的链接
function choiceShare:shareXianLiao(isClub)
    ...
    NativeCall.getInstance():callNative(data,function(phoneInfo)
        ...
        if _isChooseServerForTest then
            -- package_name = "com.CvguLuZAAHod.OljLSm.jhbAH"                -- 写测试包名就不需要企业签可以测试闲聊分享房间
        end
        ...
        HttpManager.getURL(reqShareUrl, function(recData)
            ...
            if not phpData.data then
                Log.e("choiceShare:shareXianLiao php data is Null url: " .. reqShareUrl)
                return
            end

            local function decode_str(str)
                if not str then
                    Log.e("choiceShare:shareXianLiao decode_str is Null url: " .. reqShareUrl)
                    return ""
                end
            ...
            if isClub then
                self:shareClub(shareData.link)
            else
                self:shareRoom(shareData)
            end
        end)
    end)
end
-- 闲聊分享房间
function choiceShare:shareRoom(shareData)
    local roomInfo=kFriendRoomInfo:getRoomBaseInfo()
    roomInfo = clone(roomInfo)
    ...
    local data = {};
    roomInfo.shareTitle = shareData.title or roomInfo.shareTitle
    roomInfo.shareDesc = shareData.desc or roomInfo.shareDesc
    data.title, data.desc = getWxShareInfo(roomInfo, playerInfo, selectSetInfo)
    data.url = shareData.link or ""                      -- 已经废弃的url地址
...
-- 闲聊分享文本内容,分享落地页为相同的分享方式
function choiceShare:shareClub(url)
    ...
    if url then
        ...
        data.url = (data.url or "")..WeChatShared._sharePath
    end
    NativeCall.getInstance():callNative(data, function(data)
