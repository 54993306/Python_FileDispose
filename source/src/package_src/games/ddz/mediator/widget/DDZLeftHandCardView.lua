...
local DDZLeftHandCardView = class("DDZLeftHandCardView", function ()
    return display.newLayer();
end);

local HAND_LAYER_POSITIONX = 50
...
--返回值：      无
--widget：      ui
function DDZLeftHandCardView:ctor( widget)
    Log.i("--wangzhi--创建左家手牌区--")
...
--函数功能：    析构函数
--返回值：      无
function DDZLeftHandCardView:dtor()
...
--函数功能：    ui关闭函数
--返回值：      无
function DDZLeftHandCardView:close()
    for _,v in pairs(self.listeners) do
        HallAPI.EventAPI:removeEvent(v)
    end
    self.listeners = nil
...
-- 增加地主手牌
function DDZLeftHandCardView:addLordIdCards(cards, seat)
    Log.i("--wangzhi--seat--",seat)
    if seat ~= DDZConst.SEAT_LEFT  then
        return
    end
    Log.i("--wangzhi--DDZLeftHandCardView--增加地主手牌--",cards)
...
function DDZLeftHandCardView:dealCard(cards, isReconnect)
    -- 这里只是为了处理战绩回放,重连不处理,避免对局中有重连，导致牌局异常
    if isReconnect then
        return
    end

    local seat = DDZConst.SEAT_LEFT
    local delCard = {}
    ...
    Log.i("--wangzhi--DDZLeftHandCardView--",cards)
    for i,v in ipairs(cards) do
        local cardType, cardValue = DDZCard.cardConvert(v);
        local cardView = PokerCardView.new(cardType, cardValue,v);
        cardView:setScale(CARD_SCALE)
...
--函数功能：    根据牌值手牌排序
--返回值：      无
function DDZLeftHandCardView:sortByValue()
...
--返回值:         无
--isUpateAll ：   是否更新所有牌
function DDZLeftHandCardView:updatePoker()
...
--函数功能：      重设牌位置
--返回值:         无
function DDZLeftHandCardView:resetPosition()
...
--cards：       要删除手牌
--seat：        删除牌的座位
function DDZLeftHandCardView:onDelCard(cards, seat)
    if seat ~= DDZConst.SEAT_LEFT  then
        return
    end
