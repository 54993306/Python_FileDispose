...
function FriendTotalOverView:addResultItems(listView, tmpData)
  ...
  -- 自摸次数
    local zmPath = tmpData.zm > 0 and imgBGWin or imgBGFail
    self:addCustomItem(listView, "自摸次数", tmpData.zm, zmPath)

 -- 杠总数
    local lGangPath = tmpData.gangTotalCount <= 0 and imgBGWin or imgBGFail
    self:addCustomItem(listView, "杠牌次数", tmpData.gangTotalCount, lGangPath)

    --翻出马总数
    local lFanMaPath = tmpData.zhongMaCount <= 0 and imgBGWin or imgBGFail
    self:addCustomItem(listView, "中马总数", tmpData.zhongMaCount, lFanMaPath)
