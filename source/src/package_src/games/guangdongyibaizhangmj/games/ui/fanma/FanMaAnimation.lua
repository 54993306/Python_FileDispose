...
function FanMaAnimation:addCache()
    cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/guangdongyibaizhangmj/fanma/fangma.plist")
    cc.SpriteFrameCache:getInstance():addSpriteFrames("package_res/games/guangdongyibaizhangmj/fanma/fanmalight.plist")
end
...
function FanMaAnimation:createBg()
    ...
    for i,v in pairs(data) do
        ...
        if #data > 1 then
            posY = display.cy+((#data/2)-(i-1))*130 - 100
...
function FanMaAnimation:drawFanMaText(bgLayer)
    local fmt = display.newSprite("package_res/games/guangdongyibaizhangmj/fanma/fanma_text.png")
    fmt:addTo(bgLayer)
