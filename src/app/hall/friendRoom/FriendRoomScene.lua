...
function FriendRoomScene:onCloseRoom()
    ...
    local userID = kUserInfo:getUserId()
    local isRoom = kFriendRoomInfo:isRoomMain(userID)
    if isRoom and (kFriendRoomInfo:getRoomInfo().teI == 0 or not IsPortrait) then--如果是房主
...
function FriendRoomScene:recvRoomQuit(packetInfo)
    ...
    if packetInfo.re == 1 then
        ...
        --如果是房主退出, 房间已解散
        if not kFriendRoomInfo:isRoomMain(exitUserID) or (IsPortrait and kFriendRoomInfo:getRoomInfo().teI ~= 0) then
