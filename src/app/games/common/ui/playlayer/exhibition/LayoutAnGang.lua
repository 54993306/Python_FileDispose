...
function LayoutAnGang:onShow()
	...
	for i=1, #self.content.mjs  do
		--优先判定是否有听，有听的状态下就跟正常的暗杠显示一样，翻出一张牌。
		if self.isTing==1 then
		...
		elseif _isTingShowAnGangCards and _isTingShowAnGangCards==true then
			--Log.i("进入报听翻暗杠牌操作。")
			if self.isShowAnGangCard == true and (self.content.operator == enSiteDirection.SITE_MYSELF or playSystem:getIsAnGangShow()) then
				-- 最后一个显示麻将字
...
function LayoutAnGang:onClose()
	...
	self:dtor()  	--释放资源时，移除注册的监听。否则第二局会有监听报错。
