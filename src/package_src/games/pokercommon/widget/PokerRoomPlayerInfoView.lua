...
local PokerClippingNode = require("package_src.games.pokercommon.commontool.PokerClippingNode")
...
function PokerRoomPlayerInfoView:ctor(data)
    self.super.ctor(self, "package_res/games/pokercommon/playerinfo.csb", data);
end
-----------------------------------------
-- 函数功能：    初始化游戏数据
-- 返回值：      无
...
function PokerRoomPlayerInfoView:onInit()
    ...
    if not url or string.len(url) < 4 then
        local headFile = self.userSex == 0 and "package_res/games/pokercommon/head/defaultHead_male.png" or "package_res/games/pokercommon/head/defaultHead_female.png"

        -- local headFile = "package_res/games/pokercommon/head/default_icon.png"
        self.head = PokerClippingNode.new("package_res/games/pokercommon/head/cicleHead.png",headFile, nHeadSize)
    ...
    PokerUtils:updateHead(fileName, url, "package_res/games/pokercommon/head/cicleHead.png", nHeadSize, self.frame, self.head, eTagClipHead)
end
...
function PokerRoomPlayerInfoView:initDistance()
    ...
    for i,v in ipairs(playerInfos) do
        ...
        if v:getProp(BasePlayerDefine.USERID) ~= self.userid  and index <= 2 then
...
function PokerRoomPlayerInfoView:onClickChatProp(pWidget, EventType)
    if EventType == ccui.TouchEventType.ended then
        ...
        if HallAPI.DataAPI:getMoney() > 0 then
            ...
            HallAPI.ViewAPI:showToast("您的背包无钻石请存放钻石");
        end
    end
end
