...
function DDZAnimView:showCardTypeAnim(seat, cardType, cardLenght)
    ...
    elseif cardType == DDZCard.CT_THREE_LINE_TAKE_DOUBLE then
        if cardLenght == 5 then
            self:showShunAnim("package_res/games/pokercommon/anim/image/threeTakeTwo.png",seat)
        else
            if isPlay then
                kPokerSoundPlayer:playEffect("feiji")
            end
...
function DDZAnimView:showGameAnim(seat, type)
    self.pan_game:stopAllActions()
    self.pan_game:removeAllChildren()
    if type == 1 then
...
function DDZAnimView:showSpring()
...
function DDZAnimView:showShunAnim(file,seat)
    ...
    if seat == DDZConst.SEAT_RIGHT then
        pos = cc.p(pos.x - ((#cards-1)*self:getSpace(#cards)+DDZCard.WIDTH)/2-self.handCardDis - self.handOffsetX,pos.y)
